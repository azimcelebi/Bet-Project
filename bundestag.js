...
class Bundestag {
    ...
    load(show = 0) {
        fetch(this.URL)
            ...
            document.getElementById("video").addEventListener('ended', () => {
                if (bundestag.INDEX < bundestag.ITEMS.length - 1) {
                    bundestag.setCurrent(bundestag.INDEX + 1)
                    bundestag.ensureInView(document.getElementById('toc'), document.getElementById('item-' + bundestag.INDEX))
                } else if (bundestag.NEXT) {
                    bundestag.load(0)
                }
            });
            if (bundestag.INDEX == bundestag.ITEMS.length - 1) {
                bundestag.setCurrent(0)
                bundestag.ensureInView(document.getElementById('toc'), document.getElementById('item-' + bundestag.INDEX))
            }
    }
    ...
    addToToc(toc, itemData, index) {
        toc.insertAdjacentHTML('beforeend', `<li id="item-${index}" onclick="bundestag.setCurrent(${index}); bundestag.ensureInView(document.getElementById('toc'), document.getElementById('item-${index}'));">
            <strong>${itemData.title}</strong><br>
            <em>${itemData.description}</em><br>
            <small>${itemData.date.toLocaleDateString('de-DE', { year: 'numeric', month: 'long', day: 'numeric' })}</small>
        </li>`);
    }
    ...
    setCurrent(index) {
        ...
        document.getElementById("title").textContent = this.ITEMS[index].title
        document.getElementById("description").textContent = this.ITEMS[index].description
        

        document.querySelectorAll("li").forEach(item => {
            item.style.backgroundColor = "";
        });
        document.getElementById("item-" + index).style.backgroundColor = "rgb(94, 4, 132)"; 
        before()
        next()
    }
}
...
// TODO beim click auf die Buttons wird beim ersten mal 
function before() {
    document.getElementById("before").onclick = () => {
        if (bundestag.INDEX > 0) {
            bundestag.setCurrent(bundestag.INDEX - 1)
            bundestag.ensureInView(document.getElementById('toc'), document.getElementById('item-' + bundestag.INDEX))
        } else if (bundestag.PREV) {
            bundestag.load(1)
        }
    }
}
